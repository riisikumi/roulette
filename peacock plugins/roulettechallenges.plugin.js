...
function initPlugin(controller) {
  ...
  for (const challenge of locationChallenges) {
    if (!controller.challengeService.hasGroup(
      groupName,
  ...
  const pluginPath = controller.smf.lastDeploy.lastServerSideStates?.peacockPlugins?.find?.(
    (p) => p.includes("challengesexample.plugin.js")
  );
  if (!pluginPath) {
    return;
  }
